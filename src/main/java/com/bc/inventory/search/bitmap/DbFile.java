...
class DbFile {
    ...
    static class Writer implements AutoCloseable {
        ...
        void write(List<Entry> indexRecords, List<ImmutableRoaringBitmap> bitmaps) throws IOException {
            writeHeader();
        ...
        void writeIndex(List<Entry> indexRecords, List<ImmutableRoaringBitmap> bitmaps) throws IOException {
            dos.writeInt(indexRecords.size());
            dos.writeInt(bitmaps.size());
            int bitmapBytes = 0;
            for (ImmutableRoaringBitmap roaringBitmap : bitmaps) {
                bitmapBytes += roaringBitmap.serializedSizeInBytes();
            }
            dos.writeInt(bitmapBytes);
